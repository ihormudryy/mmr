...
class TraderServiceApi(RPCHandler):
    """Read/query RPC surface for the legacy dill/msgpack transport
    (``clientserver.RPCServer``) AND the base class ``TraderServiceApi``
    handlers ``production_api.build_production_registry`` wraps onto the
    typed ``query`` socket.

    The direct-order and risk-mutation methods that used to live here
    (``place_order_simple``, ``place_expressive_order``,
    ``place_standalone_order``, ``set_risk_limits``, ``cancel_all``) moved to
    ``trader.messaging.legacy_offline_api.LegacyOfflineTraderServiceApi`` (a
    subclass of this class) -- see that module's docstring for why. This
    class itself no longer exposes any order-mutating capability.
    """
