...
- `trader_service` is the sole production writer of `trade_proposals`, `command_ledger`, `command_audit`, and `trading_control_state`. Every durable mutation and its journal event commit in one explicit transaction; entities whose materialized table is mutated by the repository itself pass a documented no-op `write_materialized` to `DomainJournal.mutate` within that same transaction.
