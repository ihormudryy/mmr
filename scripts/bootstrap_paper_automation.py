...
def _build_and_export(signer: AttestationSigner, artifacts_root: Path) -> str:
...
def main() -> int:
    ...
    private_key = args.config_dir / "keys" / "private" / "signing.pem"
    key_ring = args.config_dir / "keys" / "verify"
    public_key = key_ring / "paper-automation.pem"
    artifacts_root = args.share_dir / "artifacts"

    _write_private_key(private_key, force=args.force)
    signer = AttestationSigner.from_key_file(str(private_key))
    _write_public_key(signer, public_key, force=args.force)

    artifact_id = _build_and_export(signer, artifacts_root)
