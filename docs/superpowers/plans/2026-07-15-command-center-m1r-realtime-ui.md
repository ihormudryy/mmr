...
- Consumes frozen interfaces from the plan index without redefining them: `DomainEvent`, `SnapshotWithCursor`, typed methods `snapshot_with_cursor`/`read_domain_events(after_cursor, limit, wait_ms)` (`[M1-F1]`), `get_quotes_snapshot` (`[M1-F2]` — it owns the quote plane; use a fake until F2 is fenced), `TypedRpcClient.call(method, body, response_model)` on query port `42101` and feed port `42103` (`[G0]`), and the `[M1-F2]` guarantee that every open position has PubSub quote coverage. Development may run against fakes per the delivery graph, but the release gate runs against real fenced producers.
...
**Interfaces:**
- Consumes: `TypedRpcClient.call(method, body, response_model)` (`[G0]`) over query `tcp://<trader>:42101` and feed `tcp://<trader>:42103`; typed methods `snapshot_with_cursor` (raises/returns error code `SNAPSHOT_NOT_READY` once a complete broker generation exists — the gate is defined by `[M1-F1]` Task 4 but DORMANT there, returning `broker_generation=0`, and ACTIVATED by `[M1-F2]`), `read_domain_events(after_cursor, limit, wait_ms) -> ReadDomainEventsResult` with fields `events: tuple[DomainEvent, ...]` and `newest_cursor: int` (`[M1-F1]` Task 5), and `get_quotes_snapshot() -> {"quotes": {instrument_id: quote_row}}` (`[M1-F2]` — the quote-plane owner; fake it until F2 is fenced).
