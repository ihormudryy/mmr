...
- [ ] Write a test with a blocking handler guarded by threading events and prove a second async heartbeat/query completes while the first handler is blocked.
- [ ] Write cancellation/shutdown tests proving at most the configured five-second drain is awaited and no response is emitted after socket close.
- [ ] Run the focused tests and observe the blocking behavior.
- [ ] Extend `TypedRpcRegistry.register(..., execution="inline")`; make `TypedRpcServer` invoke `thread` handlers through `asyncio.to_thread` while preserving schema validation and authenticated request context.
- [ ] Mark every coordinator, broker snapshot, proposal, cancel, control, and strategy-forwarding registration `execution="thread"`.
- [ ] Add a bounded per-server semaphore (`TYPED_RPC_MAX_IN_FLIGHT`, default 32) and return `SERVER_BUSY` without invoking a handler when saturated.
- [ ] Run `uv run --frozen --extra test pytest tests/test_typed_rpc_async_handlers.py tests/test_typed_rpc.py tests/test_command_stack.py -q` and commit `fix(command-plane): isolate blocking command work`.
...
- [ ] Add tests that insert two promoted broker generations and prove account, positions, and working orders all come from the latest single promoted generation/cursor.
- [ ] Add tests for no promoted generation, account mismatch, mode mismatch, staging generation, generation regression, non-finite net liquidation/P&L, and a concurrent writer committing during the open read transaction.
- [ ] Update approval-context tests to require `generation_id`, `source_cursor`, `promoted_at`, and authoritative `account_mode`; remove the “one call per live port means one generation” assertion.
- [ ] Run the focused tests and confirm they fail against the current sequential `BrokerAuthority` reads.
- [ ] Implement the store read inside one DuckDB read transaction. Select the latest `status='promoted'` row, validate its cursor, and read all materialized account/position/order rows before commit.
- [ ] Implement `TraderBrokerRiskSnapshotAuthority` over the trader-owned journal connection. Quotes and IB what-if remain independent evidence and must not be labeled with the broker generation.
- [ ] Refactor `ApprovalContext` to contain `broker: BrokerRiskSnapshot`, `market: ExecutableMarketEvidence`, and optional timestamped `what_if`; retain compatibility properties only inside the module while callers migrate in this task.
- [ ] Run `uv run --frozen --extra test pytest tests/test_broker_risk_snapshot.py tests/test_approval_context.py tests/test_command_ports.py tests/test_approval_command_service.py -q` and commit `fix(risk): capture fenced broker approval evidence`.
...
- [ ] Add table-driven failures for stale/missing/crossed/non-finite quote, delayed live quote, halt, session closed, broker not ready, generation regression, account/mode mismatch, maximum notional, drift, pause, and missing live what-if.
- [ ] Add a TOCTOU test: initial validation passes, quote becomes stale or position changes before dispatch, no call reaches `OrderDispatch`.
- [ ] Add a monotonic reducing invariant property test: for any signed position/quantity, a reduction is permitted only when the result keeps the sign or reaches zero and strictly reduces absolute exposure.
- [ ] Run failing tests.
- [ ] Implement explicit `RiskDecision` reason codes and persist both initial and dispatch-time decisions before side effects.
- [ ] Re-capture broker snapshot and executable quote immediately before dispatch. Reject if generation regresses, the target position/order state materially changes, or freshness/policy fails.
- [ ] Live mode fails closed when what-if is unavailable; paper behavior follows explicit config and records `WHAT_IF_UNAVAILABLE_PAPER`, never zero.
- [ ] Run focused tests and commit `feat(risk): enforce pre-dispatch evidence revalidation`.
...
- [ ] Write tests proving `pause_trading` accepts no caller-supplied `is_live`, requires a reason, does not require preflight, and is idempotent.
- [ ] Write tests proving `resume_trading` derives live/paper from the pinned trader account, requires expected control revision, live preflight where applicable, current readiness, and complete reconciliation.
- [ ] Test that a paper browser cannot label a live trader command as paper and that account ID in a request body is ignored/rejected.
- [ ] Replace `set_trading_pause` with explicit typed actions. Preserve a read-only compatibility alias only if an existing caller needs it; it must not be registered on production command sockets.
- [ ] Change the UI to use pause/resume endpoints and show authoritative correlated state, never HTTP 202 as completion.
- [ ] Run `uv run --frozen --extra test pytest tests/test_pause_resume_authority.py tests/test_dashboard_commands.py tests/test_command_policy.py -q` and commit `fix(control): separate safe pause from guarded resume`.
...
- [ ] Add state-machine tests for immediate triggers, rolling thresholds (3 quote failures/5m, 3 ambiguous outcomes/5m, 5 runtime exceptions/10m), disconnect grace, restart persistence, duplicate trigger idempotency, and next-session daily-loss reset restriction.
- [ ] Add semantic readiness tests requiring: IB connected/account pinned, current promoted broker generation, journal writable, reconciliation backlog safe, pause/control readable, breaker clear, current XNYS session, command stack active, and quote readiness for configured instruments.
- [ ] Run tests and confirm missing implementation.
- [ ] Implement `CircuitBreakerStore` mutations and journal events atomically. Use injected clocks; no wall-clock calls inside policy methods.
- [ ] Implement `CircuitBreaker.record(signal)` and `reset(command_id, reason, operator_authority)`; reset fails unless semantic readiness and reconciliation pass.
- [ ] Expose separate liveness and semantic readiness fields through `get_status`; make Compose healthcheck require liveness only, while automation requires semantic readiness.
- [ ] Run focused tests and commit `feat(safety): add durable automation circuit breaker`.
...
- [ ] Test cancel-before-reduce ordering, partial fills, position changes during liquidation, repeated calls, disconnect, timeout, restart rescan, external working orders, and a would-flip quantity.
- [ ] Assert `FLAT` requires a fresh promoted broker snapshot with zero positions and no working orders; an HTTP/RPC acknowledgement or submitted market order is insufficient.
- [ ] Implement child command IDs deterministically under the root cause ID and use existing cancel/dispatch/reconciliation adapters.
- [ ] Keep breaker tripped for all non-`FLAT` terminal/ambiguous results. Never fabricate zero positions.
- [ ] Start rescan before readiness and continue until broker evidence resolves.
- [ ] Run focused/integration tests and commit `feat(safety): add broker-verified liquidation saga`.
...
- [ ] Add scripted drills for stale quote, trader delay, broker disconnect, ambiguous submit, partial fill, rejected protection hook, restart with unresolved command, pause/resume, and liquidation.
- [ ] Assert every drill leaves one idempotent command history, durable audit, no duplicate order reference, and a coherent breaker/readiness state.
- [ ] Update default config documentation with every activation prerequisite; keep commands disabled by default.
...
- [ ] Commit `test(command-plane): add production activation and recovery gates`.
